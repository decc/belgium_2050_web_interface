...
module Helper
  ...
  def classic_table_row_for_choice(choice)
    row = ["<td class='name'><a href='#' onmouseover='twentyfifty.startDemo(#{choice.number}); return true;' onmouseout='twentyfifty.stopDemo(#{choice.number});return true;'>#{choice.name}<a/></td>"]
